...
@Component
public class ShipmentReadSyncBasicAuthenticationFilter extends OncePerRequestFilter {

    private static final String ENDPOINT_PREFIX = "/internal/shipments/read-sync/";
    private static final String BASIC_PREFIX = "Basic ";

    private final UserReadRepository userReadRepository;
    private final PasswordEncoder passwordEncoder;

    public ShipmentReadSyncBasicAuthenticationFilter(final UserReadRepository userReadRepository,
    ...
    private boolean hasRefreshPermission(final UserEntity user) {
        final Set<String> permissions = user.getPermissions().stream()
                .map(RolePermissionEntity::getName)
                .collect(Collectors.toSet());
    ...
    private void unauthorized(final HttpServletResponse response) throws IOException {
        ...
        response.setHeader(HttpHeaders.WWW_AUTHENTICATE, "Basic realm=\"shipment-read-sync\"");
