...
public class OperatorUserNotifyAdapter implements OperatorUserNotifyPort {
    ...
    @Override
    public UserId notifyOperatorCreated(final OperatorSnapshot snapshot, final Consumer<UserId> beforeUserCreated) {
        final AtomicReference<UserId> userCreatedId = new AtomicReference<>();
        final OperatorCreatedEvent event = OperatorUserNotifyMapper.toEvent(
                snapshot,
                beforeUserCreated,
                userCreatedId::set
        );
